...
let STATE = null;
let fbApp = null, fbDb = null, fbUnsub = null, fbUnsubRegistros = null, fbConnected = false;
let saveTimer = null;
...
// persist(): guarda todo en localStorage y sincroniza el estado "núcleo"
// (actividades, sesiones, nomenclatura, sanciones, contraseñas) como UN
// solo documento en Firebase. Esto es seguro porque normalmente solo el
// staff edita estas cosas, uno a la vez.
//
// Los REGISTROS de estudiantes son distintos: muchos estudiantes pueden
// registrarse al mismo tiempo desde dispositivos distintos, así que si
// se guardaran dentro de ese mismo documento, dos escrituras casi
// simultáneas podrían pisarse una a la otra y borrar registros sin que
// nadie se diera cuenta. Por eso cada registro se guarda como un
// documento independiente (ver persistRegistro), y nunca se sobrescriben
// entre sí.
function persist(){
  ...
  if (fbConnected && fbDb){
    ...
    saveTimer = setTimeout(() => {
      const core = { ...STATE };
      delete core.registros;
      fbDb.collection("cai").doc("estado").set(core).catch(err => {
...
// Crea o actualiza UN registro individual, sin tocar los demás.
function persistRegistro(reg){
  STATE.updatedAt = Date.now();
  saveLocal();
  if (fbConnected && fbDb){
    fbDb.collection("cai").doc("estado").collection("registros").doc(reg.id).set(reg).catch(err => {
      toast("No se pudo sincronizar el registro: " + err.message, "error");
    });
  }
  renderAll();
}

// Borra uno o varios registros individuales remotos (usado al eliminar
// una sesión o actividad completa).
function deleteRegistrosRemote(ids){
  if (fbConnected && fbDb && ids.length){
    const col = fbDb.collection("cai").doc("estado").collection("registros");
    ids.forEach(id => col.doc(id).delete().catch(() => {}));
  }
}
...
function initState(){
  ...
  // Prioridad: config incrustada en data.js (FIREBASE_CONFIG) SIEMPRE
  // gana si existe — es la fuente de la verdad para todos los
  // dispositivos. Solo se usa la config guardada en este navegador si no
  // hay ninguna incrustada en el código (caso de pruebas antes de tener
  // un proyecto de Firebase definitivo). Esto evita que un dispositivo
  // con una configuración vieja o incompleta guardada por accidente se
  // quede "atascado" sin poder sincronizar, sin forma de arreglarlo
  // (los estudiantes no tienen acceso a la pantalla de Configuración).
  const fbCfg = (typeof FIREBASE_CONFIG !== "undefined" && FIREBASE_CONFIG) || loadFbConfig();
...
function connectFirebase(cfg, silent){
  try{
    ...
    if (fbUnsub) fbUnsub();
    if (fbUnsubRegistros) fbUnsubRegistros();

    const estadoRef = fbDb.collection("cai").doc("estado");
    const registrosRef = estadoRef.collection("registros");

    fbUnsub = estadoRef.onSnapshot(
      (doc) => {
        fbConnected = true;
        setSyncBadge(true);
        if (doc.exists){
          ...
          if (!STATE || (remote.updatedAt || 0) >= (STATE.updatedAt || 0)){
            const registrosActuales = STATE ? STATE.registros : [];
            STATE = { ...remote, registros: registrosActuales };
            if (!STATE.docentePasswordHash) STATE.docentePasswordHash = hashStr(DEFAULT_DOCENTE_PASSWORD);
            saveLocal();
            renderAll();
          }
        } else {
          // Primera vez que se conecta este proyecto de Firebase: sube el
          // estado núcleo y migra los registros que ya existieran localmente
          // (cada uno como documento independiente).
          const core = { ...STATE };
          delete core.registros;
          estadoRef.set(core);
          (STATE.registros || []).forEach(r => registrosRef.doc(r.id).set(r));
        }
      },
      (err) => {
        fbConnected = false;
    ...
    fbUnsubRegistros = registrosRef.onSnapshot(
      (snap) => {
        STATE.registros = snap.docs.map(d => d.data());
        saveLocal();
        renderAll();
      },
      (err) => {
        if (!silent) toast("Error al sincronizar registros: " + err.message, "error");
      }
    );
...
function disconnectFirebase(){
  if (fbUnsub) fbUnsub();
  if (fbUnsubRegistros) fbUnsubRegistros();
  fbUnsub = null; fbUnsubRegistros = null; fbDb = null; fbConnected = false;
  clearFbConfig();
  setSyncBadge(false);
...
function openDocenteGate(targetTab){
  openModal(`
    ...
    <p class="muted-sm">Introduce la contraseña que te compartió el CAI para consultar la asistencia de tus grupos. Esta vista es solo de lectura.</p>
    <label>Contraseña
  ...
  const tryEnter = () => {
    ...
    if (hashStr(pass) === STATE.docentePasswordHash){
...
function deleteActividad(id){
  ...
  document.getElementById("delConfirm").addEventListener("click", () => {
    ...
    const registrosAEliminar = STATE.registros.filter(r => sesionIds.includes(r.sesionId)).map(r => r.id);
    ...
    deleteRegistrosRemote(registrosAEliminar);
    persist();
    closeModal();
...
function deleteSesion(id){
  ...
  document.getElementById("dsConfirm").addEventListener("click", () => {
    const registrosAEliminar = STATE.registros.filter(r => r.sesionId === id).map(r => r.id);
  ...
  reader.onload = () => {
    try{
      ...
      if (fbConnected && fbDb){
        ...
        const core = { ...STATE };
        delete core.registros;
        estadoRef.set(core).catch(err => toast("No se pudo sincronizar el respaldo: " + err.message, "error"));
        // Reemplaza también la colección remota de registros completa
        registrosRef.get().then(snap => {
          const nuevosIds = new Set(STATE.registros.map(r => r.id));
          snap.docs.forEach(d => { if (!nuevosIds.has(d.id)) d.ref.delete().catch(() => {}); });
          STATE.registros.forEach(r => registrosRef.doc(r.id).set(r).catch(() => {}));
        }).catch(() => {});
      }
...
function setupConfigTab(){
  ...
  document.getElementById("wipeBtn").addEventListener("click", () => {
    ...
    document.getElementById("wipeConfirm").addEventListener("click", () => {
      const idsAEliminar = STATE.registros.map(r => r.id);
      STATE = defaultState();
      deleteRegistrosRemote(idsAEliminar);
      persist();
      closeModal();
