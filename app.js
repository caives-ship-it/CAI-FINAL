...
const LS_DOCENTE_OK = "cai_uvp_docente_ok"; // sessionStorage
const LS_DOCENTE_NAME = "cai_uvp_docente_name"; // sessionStorage — nombre del docente autenticado individualmente (vacío si entró con la contraseña general)
...
function defaultState(){
  return {
    ...
    docentePasswordHash: hashStr(DEFAULT_DOCENTE_PASSWORD),
    ...
    docentes: [], // { id, nombre, passwordHash } — acceso individual al reporte de docentes
    academicPeriods: [], // { id, nombre, desde (ISO), hasta (ISO) }
    updatedAt: Date.now(),
  };
}
...
function migrateState(){
  if (!STATE.docentePasswordHash) STATE.docentePasswordHash = hashStr(DEFAULT_DOCENTE_PASSWORD);
  if (!Array.isArray(STATE.docentes)) STATE.docentes = [];
  if (!Array.isArray(STATE.academicPeriods)) STATE.academicPeriods = [];
}
  ...
  try{
    ...
    fbUnsub = fbDb.collection("cai").doc("estado").onSnapshot(
      (doc) => {
        ...
        setSyncBadge(true);
        if (doc.exists){
          // Firestore es siempre la fuente de verdad compartida: se acepta
          // tal cual, sin comparar fechas. (Antes se comparaba updatedAt,
          // pero el estado "vacío" que se genera al abrir la app por
          // primera vez siempre queda con una fecha más reciente que
          // cualquier dato real ya guardado, así que nunca se sincronizaba
          // para quien entraba por primera vez.)
          STATE = doc.data();
          migrateState();
          saveLocal();
          renderAll();
...
const DOCENTE_ONLY_TABS = ["docentes"];

function isStaff(){ return sessionStorage.getItem(LS_STAFF_OK) === "1"; }
function isDocenteAuthed(){ return sessionStorage.getItem(LS_DOCENTE_OK) === "1"; }
function canViewDocentes(){ return isStaff() || isDocenteAuthed(); }

function goTab(tabId){
  if (GUARDED_TABS.includes(tabId) && !isStaff()) tabId = "registro";
  if (DOCENTE_ONLY_TABS.includes(tabId) && !canViewDocentes()) tabId = "registro";
  document.querySelectorAll(".tab-btn").forEach(b => b.classList.toggle("active", b.dataset.tab === tabId));
  document.querySelectorAll(".tab-panel").forEach(p => p.classList.toggle("active", p.id === "tab-" + tabId));
  window.location.hash = tabId;
  const noBanner = tabId === "registro" || tabId === "docentes";
  document.getElementById("syncWarning").classList.toggle("hidden", fbConnected || noBanner);
  if (GUARDED_TABS.includes(tabId) || DOCENTE_ONLY_TABS.includes(tabId)) renderTab(tabId);
}
...
function setupNav(){
  document.querySelectorAll(".tab-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (btn.dataset.docenteGuarded !== undefined && !canViewDocentes()){
        openDocenteGate(tabId);
        return;
      }
      goTab(tabId);
    });
...
function openDocenteGate(targetTab){
  const tieneDocentes = STATE.docentes.length > 0;
  const opts = STATE.docentes.slice().sort((a,b)=>a.nombre.localeCompare(b.nombre))
    .map(d => `<option value="${d.id}">${esc(d.nombre)}</option>`).join("");
  openModal(`
    <h2>Reporte para docentes</h2>
    <p class="muted-sm">${tieneDocentes
      ? "Selecciona tu nombre e introduce tu contraseña para consultar la asistencia de tus grupos. Esta vista es solo de lectura y solo muestra tu información."
      : "Introduce la contraseña que te compartió el CAI para consultar la asistencia de tus grupos. Esta vista es solo de lectura."}</p>
    ${tieneDocentes ? `<label>Docente
      <select id="docNombre">${opts}</select>
    </label>` : ""}
    <label>Contraseña
      <input type="password" id="docPass" autocomplete="off">
    </label>
    <div id="docErr" class="form-msg error hidden"></div>
    <div class="btn-row">
      <button class="btn-secondary" id="docCancel">Cancelar</button>
      <button class="btn-primary" id="docEnter">Entrar</button>
    </div>
  `);
  document.getElementById("docCancel").addEventListener("click", closeModal);
  const tryEnter = () => {
    const pass = document.getElementById("docPass").value;
    const err = document.getElementById("docErr");
    if (tieneDocentes){
      const sel = document.getElementById("docNombre");
      const docente = STATE.docentes.find(d => d.id === sel.value);
      if (docente && hashStr(pass) === docente.passwordHash){
        sessionStorage.setItem(LS_DOCENTE_OK, "1");
        sessionStorage.setItem(LS_DOCENTE_NAME, docente.nombre);
        closeModal();
        goTab(targetTab);
        return;
      }
    } else if (hashStr(pass) === STATE.docentePasswordHash){
      sessionStorage.setItem(LS_DOCENTE_OK, "1");
      sessionStorage.removeItem(LS_DOCENTE_NAME);
      closeModal();
      goTab(targetTab);
      return;
    }
  ...
  document.getElementById("docEnter").addEventListener("click", tryEnter);
  document.getElementById("docPass").addEventListener("keydown", e => { if (e.key === "Enter") tryEnter(); });
  document.getElementById("docPass").focus();
}
...
function actividadesConCupo(){
  const idsConCupo = new Set(upcomingOpenSesiones().map(s => s.actividadId));
  return STATE.actividades.filter(a => idsConCupo.has(a.id));
}

function renderRegistroForm(){
  const datalist = document.getElementById("actividadesDatalist");
  datalist.innerHTML = actividadesConCupo()
    .slice().sort((a,b)=>a.nombre.localeCompare(b.nombre))
    .map(a => `<option value="${escAttr(a.nombre)}">`).join("");

  // re-evalúa la actividad ya escrita (si el usuario tenía texto y cambiaron los cupos)
  updateSesionOptionsForActividad(document.getElementById("f_actividad_busca").value);
...
function updateSesionOptionsForActividad(texto){
  const sel = document.getElementById("f_sesion");
  const preview = document.getElementById("sesionPreview");
  const nombreNormalizado = (texto || "").trim().toLowerCase();
  const match = nombreNormalizado
    ? actividadesConCupo().find(a => a.nombre.trim().toLowerCase() === nombreNormalizado)
    : null;

  preview.classList.add("hidden");

  if (!match){
    sel.innerHTML = `<option value="">${nombreNormalizado ? "— no encontramos esa actividad con cupo, revisa el nombre —" : "— primero escribe el nombre de una actividad con cupo —"}</option>`;
    sel.disabled = true;
    return null;
  }

  const sesiones = STATE.sesiones
    .filter(s => s.actividadId === match.id)
    .filter(s => upcomingOpenSesiones().some(u => u.id === s.id))
    .sort((a,b) => (a.fecha+a.hora).localeCompare(b.fecha+b.hora));

  sel.disabled = false;
  sel.innerHTML = `<option value="">— selecciona fecha y hora —</option>` + sesiones.map(s => {
    const disp = sesionCupoDisponible(s);
    return `<option value="${s.id}">${fmtFecha(s.fecha)} ${s.hora || ""} (${disp} lugar${disp===1?"":"es"})</option>`;
  }).join("");
...
function renderCatalogList(){
  ...
  wrap.innerHTML = activas.map(a => {
    ...
    return `
    <div class="catalog-item catalog-item-clickable" data-nombre="${escAttr(a.nombre)}" title="Clic para usar esta actividad en el formulario">
      <div class="tag-row">
  ...
  wrap.querySelectorAll(".catalog-item-clickable").forEach(el => {
    el.addEventListener("click", () => {
      const input = document.getElementById("f_actividad_busca");
      input.value = el.dataset.nombre;
      updateSesionOptionsForActividad(input.value);
...
function setupRegistroForm(){
  ...
  const actividadInput = document.getElementById("f_actividad_busca");
  ...
  actividadInput.addEventListener("input", () => {
    updateSesionOptionsForActividad(actividadInput.value);
  });
...
function submitRegistro(){
  ...
  updateSesionOptionsForActividad("");
}
...
function newSesionFormHtml(actividadId){
  ...
  return `
    ...
    <label class="checkbox-label">
      <input type="checkbox" id="sf_repite"> Esta actividad se repite (programar varias fechas a la vez)
    </label>
    <div id="sf_repeat_options" class="repeat-block hidden">
      <p class="muted-sm" style="margin:0 0 8px;">¿Qué día(s) de la semana se imparte?</p>
      <div class="weekday-picker">
        ${dias.map(([v,l]) => `<label><input type="checkbox" class="sf_wd" value="${v}"> ${l}</label>`).join("")}
      </div>
      <label>Repetir hasta (última fecha incluida)<input type="date" id="sf_hasta"></label>
    </div>
...
function newSesionForActividad(actividadId){
  ...
  const repiteCheck = document.getElementById("sf_repite");
  const repeatOptions = document.getElementById("sf_repeat_options");
  const fechaInput = document.getElementById("sf_fecha");
  repiteCheck.addEventListener("change", () => {
    repeatOptions.classList.toggle("hidden", !repiteCheck.checked);
    if (repiteCheck.checked){
      // marca por defecto el día de la semana de la fecha ya elegida
      const wd = String(new Date(fechaInput.value + "T00:00:00").getDay());
      const box = document.querySelector(`.sf_wd[value="${wd}"]`);
      if (box) box.checked = true;
      if (!document.getElementById("sf_hasta").value){
        document.getElementById("sf_hasta").value = addDaysISO(fechaInput.value, 60);
      }
    }
  });
  ...
  document.getElementById("sfSave").addEventListener("click", () => {
    ...
    if (repiteCheck.checked){
      const hasta = document.getElementById("sf_hasta").value;
      const diasSeleccionados = Array.from(document.querySelectorAll(".sf_wd:checked")).map(c => parseInt(c.value, 10));
      if (!hasta) return toast("Indica hasta qué fecha se repite.", "error");
      if (hasta < fecha) return toast("La fecha final debe ser posterior a la fecha de inicio.", "error");
      if (!diasSeleccionados.length) return toast("Selecciona al menos un día de la semana.", "error");

      let cursor = fecha;
      let creadas = 0;
      while (cursor <= hasta){
        const wd = new Date(cursor + "T00:00:00").getDay();
        if (diasSeleccionados.includes(wd)){
          STATE.sesiones.push({ id: uid("ses"), actividadId: actId, fecha: cursor, hora, cupo, estado: "abierta" });
          creadas++;
        }
      ...
      persist();
      closeModal();
      toast(`${creadas} fecha${creadas===1?"":"s"} programada${creadas===1?"":"s"}`, "ok");
    } else {
...
function renderEstadisticasTab(){
  ...
  // Comparativo entre periodos académicos
  renderPeriodCompareSelects();
  renderPeriodCompareTable();
}

function periodStats(desde, hasta){
  const sesionIds = STATE.sesiones.filter(s => s.fecha >= desde && s.fecha <= hasta).map(s => s.id);
  const regs = STATE.registros.filter(r => sesionIds.includes(r.sesionId));
  const asis = regs.filter(r => r.asistencia === "asistio").length;
  const noAsis = regs.filter(r => r.asistencia === "no_asistio").length;
  const tasa = regs.length ? Math.round((asis/regs.length)*100) : 0;

  const porActividad = {};
  regs.forEach(r => {
    const sesion = getSesion(r.sesionId);
    const act = sesion ? getActividad(sesion.actividadId) : null;
    const nombre = act ? act.nombre : "(eliminada)";
    porActividad[nombre] = (porActividad[nombre]||0) + 1;
  });
  const topActividad = Object.entries(porActividad).sort((a,b)=>b[1]-a[1])[0];

  return {
    sesiones: sesionIds.length,
    registros: regs.length,
    asistencias: asis,
    inasistencias: noAsis,
    tasa,
    topActividad: topActividad ? `${topActividad[0]} (${topActividad[1]})` : "—",
  };
}

function renderPeriodCompareSelects(){
  const selA = document.getElementById("cmp_periodoA");
  const selB = document.getElementById("cmp_periodoB");
  const empty = document.getElementById("periodCompareEmpty");
  const periods = STATE.academicPeriods.slice().sort((a,b)=>a.desde.localeCompare(b.desde));

  empty.classList.toggle("hidden", periods.length > 0);
  document.getElementById("periodCompareTable").classList.toggle("hidden", periods.length === 0);

  if (!periods.length) return;

  const opts = periods.map(p => `<option value="${p.id}">${esc(p.nombre)}</option>`).join("");
  const curA = selA.value, curB = selB.value;
  selA.innerHTML = opts;
  selB.innerHTML = opts;
  selA.value = periods.some(p=>p.id===curA) ? curA : periods[periods.length-1].id;
  selB.value = periods.some(p=>p.id===curB) && curB !== selA.value
    ? curB
    : (periods.length > 1 ? periods[periods.length-2].id : periods[0].id);
}

function renderPeriodCompareTable(){
  const periods = STATE.academicPeriods;
  if (!periods.length) return;
  const selA = document.getElementById("cmp_periodoA");
  const selB = document.getElementById("cmp_periodoB");
  const pA = periods.find(p => p.id === selA.value);
  const pB = periods.find(p => p.id === selB.value);
  if (!pA || !pB) return;

  document.getElementById("cmp_headA").textContent = pA.nombre;
  document.getElementById("cmp_headB").textContent = pB.nombre;

  const statsA = periodStats(pA.desde, pA.hasta);
  const statsB = periodStats(pB.desde, pB.hasta);

  const rows = [
    ["Sesiones realizadas", statsA.sesiones, statsB.sesiones],
    ["Registros totales", statsA.registros, statsB.registros],
    ["Asistencias", statsA.asistencias, statsB.asistencias],
    ["Inasistencias", statsA.inasistencias, statsB.inasistencias],
    ["% de asistencia", statsA.tasa + "%", statsB.tasa + "%"],
    ["Actividad con más demanda", statsA.topActividad, statsB.topActividad],
  ];
  document.querySelector("#periodCompareTable tbody").innerHTML = rows.map(([lbl,a,b]) => `
    <tr><td><strong>${esc(lbl)}</strong></td><td>${esc(String(a))}</td><td>${esc(String(b))}</td></tr>
  `).join("");
}
...
/* ---------------------------------------------------------------------- *
 * 10.5 TAB: DOCENTES (reporte de solo lectura)
 * ---------------------------------------------------------------------- */

function listaDocentes(){
  return [...new Set(STATE.registros.map(r => r.docente).filter(Boolean))].sort();
}

function renderDocentesTab(){
  const sel = document.getElementById("doc_filtro");
  const lockedNombre = (!isStaff() && isDocenteAuthed()) ? sessionStorage.getItem(LS_DOCENTE_NAME) : null;

  if (lockedNombre){
    // Docente con acceso individual: el filtro queda fijo en su propio nombre.
    sel.innerHTML = `<option value="${escAttr(lockedNombre)}">${esc(lockedNombre)}</option>`;
    sel.value = lockedNombre;
    sel.disabled = true;
  } else {
    const current = sel.value;
    sel.innerHTML = `<option value="">Todos los docentes</option>` +
      listaDocentes().map(d => `<option value="${escAttr(d)}">${esc(d)}</option>`).join("");
    if (current) sel.value = current;
    sel.disabled = false;
  }

  document.getElementById("docenteLogoutWrap").classList.toggle("hidden", isStaff() || !isDocenteAuthed());

  renderDocentesReporte();
}

function renderDocentesReporte(){
  const filtro = document.getElementById("doc_filtro").value;
  const regs = STATE.registros.filter(r => !filtro || r.docente === filtro);

  const total = regs.length;
  const asistieron = regs.filter(r => r.asistencia === "asistio").length;
  const noAsistieron = regs.filter(r => r.asistencia === "no_asistio").length;
  const tasa = total ? Math.round((asistieron/total)*100) : 0;

  document.getElementById("docKpis").innerHTML = `
    <div class="kpi"><div class="num">${total}</div><div class="lbl">Registros</div></div>
    <div class="kpi"><div class="num">${asistieron}</div><div class="lbl">Asistencias</div></div>
    <div class="kpi"><div class="num">${noAsistieron}</div><div class="lbl">Inasistencias</div></div>
    <div class="kpi"><div class="num">${tasa}%</div><div class="lbl">Tasa de asistencia</div></div>
  `;

  const rows = regs.slice().sort((a,b) => b.timestamp - a.timestamp).map(r => {
    const sesion = getSesion(r.sesionId);
    const act = sesion ? getActividad(sesion.actividadId) : null;
    return `
      <tr>
        <td>${esc(act ? act.nombre : "(eliminada)")}</td>
        <td>${sesion ? fmtFecha(sesion.fecha) : "—"} ${sesion ? esc(sesion.hora||"") : ""}</td>
        <td>${esc(r.nombre)}</td>
        <td><code>${esc(r.matricula)}</code></td>
        <td>${esc(r.carrera)}</td>
        <td>${esc(r.grupo)}</td>
  ...
  document.querySelector("#docTable tbody").innerHTML = rows ||
    `<tr><td colspan="7" class="muted-sm">No hay registros para este filtro todavía.</td></tr>`;
}

function setupEstadisticasTab(){
  document.getElementById("cmp_periodoA").addEventListener("change", renderPeriodCompareTable);
  document.getElementById("cmp_periodoB").addEventListener("change", renderPeriodCompareTable);
}

function setupDocentesTab(){
  document.getElementById("doc_filtro").addEventListener("change", renderDocentesReporte);
  document.getElementById("docenteLogoutBtn").addEventListener("click", () => {
    sessionStorage.removeItem(LS_DOCENTE_OK);
    sessionStorage.removeItem(LS_DOCENTE_NAME);
    toast("Saliste del reporte de docentes", "ok");
    goTab("registro");
  });
}
...
function buildResumenActividad(regsBase){
  const base = regsBase || STATE.registros;
  return STATE.actividades.map(a => {
    const sesionIds = STATE.sesiones.filter(s => s.actividadId === a.id).map(s => s.id);
    const regs = base.filter(r => sesionIds.includes(r.sesionId));
    const asis = regs.filter(r => r.asistencia === "asistio").length;
    const noAsis = regs.filter(r => r.asistencia === "no_asistio").length;
    return {
      Actividad: a.nombre,
...
function buildResumenCarrera(regsBase){
  const base = regsBase || STATE.registros;
  const porCarrera = {};
  base.filter(r => r.asistencia === "asistio").forEach(r => {
    porCarrera[r.carrera] = (porCarrera[r.carrera]||0)+1;
  });
...
function buildExportRowsFrom(regs){
  return regs.map(r => {
    const sesion = getSesion(r.sesionId);
    const act = sesion ? getActividad(sesion.actividadId) : null;
    return {
...
function renderExportarTab(){
  const selPer = document.getElementById("pdf_periodo");
  const selAct = document.getElementById("pdf_actividad");
  const selCar = document.getElementById("pdf_carrera");
  const selDoc = document.getElementById("pdf_docente");

  const curPer = selPer.value, curAct = selAct.value, curCar = selCar.value, curDoc = selDoc.value;

  selPer.innerHTML = `<option value="">Todos</option>` + STATE.academicPeriods
    .slice().sort((a,b)=>a.desde.localeCompare(b.desde))
    .map(p => `<option value="${p.id}">${esc(p.nombre)}</option>`).join("");
  selAct.innerHTML = `<option value="">Todas</option>` + STATE.actividades
    .map(a => `<option value="${a.id}">${esc(a.nombre)}</option>`).join("");
  const carreras = [...new Set(STATE.registros.map(r => r.carrera).filter(Boolean))].sort();
  selCar.innerHTML = `<option value="">Todas</option>` + carreras
    .map(c => `<option value="${escAttr(c)}">${esc(c)}</option>`).join("");
  selDoc.innerHTML = `<option value="">Todos</option>` + listaDocentes()
    .map(d => `<option value="${escAttr(d)}">${esc(d)}</option>`).join("");

  if ([...selPer.options].some(o=>o.value===curPer)) selPer.value = curPer;
  if ([...selAct.options].some(o=>o.value===curAct)) selAct.value = curAct;
  if ([...selCar.options].some(o=>o.value===curCar)) selCar.value = curCar;
  if ([...selDoc.options].some(o=>o.value===curDoc)) selDoc.value = curDoc;
}

function filteredRegistrosForPdf(){
  const periodoId = document.getElementById("pdf_periodo").value;
  const actividadId = document.getElementById("pdf_actividad").value;
  const carrera = document.getElementById("pdf_carrera").value;
  const docente = document.getElementById("pdf_docente").value;
  const periodo = periodoId ? STATE.academicPeriods.find(p => p.id === periodoId) : null;

  return STATE.registros.filter(r => {
    const sesion = getSesion(r.sesionId);
    if (actividadId && (!sesion || sesion.actividadId !== actividadId)) return false;
    if (periodo && (!sesion || sesion.fecha < periodo.desde || sesion.fecha > periodo.hasta)) return false;
...
function exportPdf(){
  if (typeof window.jspdf === "undefined"){
    toast("No se pudo cargar la librería de PDF (sin conexión). Usa exportar Excel o CSV.", "error");
    return;
  }
  const { jsPDF } = window.jspdf;
  const doc = new jsPDF({ orientation: "landscape", unit: "pt" });
  const regs = filteredRegistrosForPdf();
  ...
  const docente = document.getElementById("pdf_docente").value;
  const periodo = periodoId ? STATE.academicPeriods.find(p => p.id === periodoId) : null;
  const actividad = actividadId ? getActividad(actividadId) : null;

  const filtrosAplicados = [
    periodo ? `Periodo: ${periodo.nombre}` : null,
    actividad ? `Actividad: ${actividad.nombre}` : null,
  ...
  doc.setFontSize(16);
  doc.text("Centro de Aplicación de Idiomas — Universidad del Valle de Puebla", 40, 40);
  doc.setFontSize(11);
  doc.text(`Reporte generado el ${fmtFecha(todayISO())}`, 40, 58);
  doc.text(filtrosAplicados.length ? `Filtros aplicados: ${filtrosAplicados.join(" · ")}` : "Sin filtros — reporte completo", 40, 74);

  doc.setFontSize(12);
  doc.text(`Registros: ${regs.length}   ·   Asistencias: ${asis}   ·   Inasistencias: ${noAsis}   ·   % de asistencia: ${tasa}%`, 40, 94);

  let nextY = 110;

  doc.autoTable({
    startY: nextY,
    head: [["Matrícula","Nombre","Carrera","Docente","Grupo","Actividad","Fecha","Estado"]],
    body: buildExportRowsFrom(regs).map(r => [r.Matrícula, r.Nombre, r.Carrera, r.Docente, r.Grupo, r.Actividad, r["Fecha sesión"], r["Estado asistencia"]]),
    styles: { fontSize: 8 },
    headStyles: { fillColor: [124, 58, 174] },
    margin: { left: 40, right: 40 },
  });
  nextY = doc.lastAutoTable.finalY + 24;

  const resumenAct = buildResumenActividad(regs);
  if (resumenAct.length){
    if (nextY > 480){ doc.addPage(); nextY = 40; }
    doc.setFontSize(13);
    doc.text("Resumen por actividad", 40, nextY);
    doc.autoTable({
      startY: nextY + 10,
      head: [["Actividad","Registrados","Asistieron","No asistieron","% Asistencia"]],
      body: resumenAct.map(r => [r.Actividad, r.Registrados, r.Asistieron, r["No asistieron"], r["% Asistencia"]]),
      styles: { fontSize: 9 },
      headStyles: { fillColor: [124, 58, 174] },
      margin: { left: 40, right: 40 },
    });
  ...
  const resumenCar = buildResumenCarrera(regs);
  if (resumenCar.length){
    if (nextY > 480){ doc.addPage(); nextY = 40; }
    doc.setFontSize(13);
    doc.text("Resumen por carrera", 40, nextY);
    doc.autoTable({
      startY: nextY + 10,
      head: [["Carrera","Asistencias"]],
      body: resumenCar.map(r => [r.Carrera, r.Asistencias]),
      styles: { fontSize: 9 },
      headStyles: { fillColor: [124, 58, 174] },
      margin: { left: 40, right: 40 },
    });
  ...
  const sanciones = buildSanciones();
  if (sanciones.length){
    if (nextY > 480){ doc.addPage(); nextY = 40; }
    doc.setFontSize(13);
    doc.text("Sanciones", 40, nextY);
    doc.autoTable({
      startY: nextY + 10,
      head: [["Matrícula","Actividad","Sanción hasta","Activa"]],
      body: sanciones.map(r => [r["Matrícula"], r.Actividad, r["Sanción hasta"], r.Activa]),
      styles: { fontSize: 9 },
      headStyles: { fillColor: [124, 58, 174] },
      margin: { left: 40, right: 40 },
    });
  ...
  doc.save(`CAI_reporte_${todayISO()}.pdf`);
}
...
/* -- Docentes con acceso individual -- */

function renderDocentesTable(){
  const tbody = document.querySelector("#docentesTable tbody");
  if (!STATE.docentes.length){
    tbody.innerHTML = `<tr><td colspan="2" class="muted-sm">Aún no hay docentes con acceso individual. Se usa la contraseña general mientras tanto.</td></tr>`;
    return;
  }
  tbody.innerHTML = STATE.docentes.slice().sort((a,b)=>a.nombre.localeCompare(b.nombre)).map(d => `
    <tr>
      <td>${esc(d.nombre)}</td>
      <td>
        <button class="icon-btn" onclick="editDocente('${d.id}')">Editar / cambiar contraseña</button>
        <button class="icon-btn" onclick="deleteDocente('${d.id}')">Eliminar</button>
      </td>
    </tr>
  `).join("");
}

function docenteFormHtml(d){
  d = d || { nombre:"" };
  return `
    <h2>${d.id?"Editar":"Nuevo"} docente</h2>
    <label>Nombre completo (tal como lo escriben los alumnos)<input type="text" id="df_nombre" value="${escAttr(d.nombre)}"></label>
    <div class="grid-2">
      <label>${d.id?"Nueva contraseña":"Contraseña"}<input type="password" id="df_pass" placeholder="${d.id?"Dejar vacío para no cambiar":""}"></label>
      <label>Confirmar<input type="password" id="df_pass2"></label>
    </div>
    <div class="btn-row">
      <button class="btn-secondary" id="dfCancel">Cancelar</button>
      <button class="btn-primary" id="dfSave">Guardar</button>
    </div>
  `;
}

function newDocente(){
  openModal(docenteFormHtml());
  bindDocenteForm(null);
}
function editDocente(id){
  const d = STATE.docentes.find(x => x.id === id);
  openModal(docenteFormHtml(d));
  bindDocenteForm(d);
}
function bindDocenteForm(existing){
  document.getElementById("dfCancel").addEventListener("click", closeModal);
  document.getElementById("dfSave").addEventListener("click", () => {
    const nombre = document.getElementById("df_nombre").value.trim();
    const p1 = document.getElementById("df_pass").value;
    const p2 = document.getElementById("df_pass2").value;
    if (!nombre) return toast("El nombre es obligatorio.", "error");
    if (p1 !== p2) return toast("Las contraseñas no coinciden.", "error");
    if (!existing && !p1) return toast("Escribe una contraseña para el docente nuevo.", "error");
    if (existing){
      existing.nombre = nombre;
      if (p1) existing.passwordHash = hashStr(p1);
    } else {
      STATE.docentes.push({ id: uid("doc"), nombre, passwordHash: hashStr(p1) });
    }
    persist();
    closeModal();
    toast("Docente guardado", "ok");
  });
}
function deleteDocente(id){
  STATE.docentes = STATE.docentes.filter(d => d.id !== id);
  persist();
  toast("Docente eliminado. Volverá a usarse la contraseña general para él/ella.", "ok");
}

/* -- Periodos académicos -- */

function renderPeriodosTable(){
  const tbody = document.querySelector("#periodosTable tbody");
  if (!STATE.academicPeriods.length){
    tbody.innerHTML = `<tr><td colspan="4" class="muted-sm">Aún no hay periodos académicos configurados.</td></tr>`;
    return;
  }
  tbody.innerHTML = STATE.academicPeriods.slice().sort((a,b)=>a.desde.localeCompare(b.desde)).map(p => `
    <tr>
      <td>${esc(p.nombre)}</td>
      <td>${fmtFecha(p.desde)}</td>
      <td>${fmtFecha(p.hasta)}</td>
      <td>
        <button class="icon-btn" onclick="editPeriodo('${p.id}')">Editar</button>
        <button class="icon-btn" onclick="deletePeriodo('${p.id}')">Eliminar</button>
      </td>
    </tr>
  `).join("");
}

function periodoFormHtml(p){
  p = p || { nombre:"", desde:"", hasta:"" };
  return `
    <h2>${p.id?"Editar":"Nuevo"} periodo académico</h2>
    <label>Nombre (ej. "A26" o "Agosto-Diciembre 2026")<input type="text" id="pr_nombre" value="${escAttr(p.nombre)}"></label>
    <div class="grid-2">
      <label>Desde<input type="date" id="pr_desde" value="${escAttr(p.desde)}"></label>
      <label>Hasta<input type="date" id="pr_hasta" value="${escAttr(p.hasta)}"></label>
    </div>
    <div class="btn-row">
...
function newPeriodo(){
  openModal(periodoFormHtml());
  bindPeriodoForm(null);
}
function editPeriodo(id){
  const p = STATE.academicPeriods.find(x => x.id === id);
  openModal(periodoFormHtml(p));
  bindPeriodoForm(p);
}
function bindPeriodoForm(existing){
  ...
  document.getElementById("prSave").addEventListener("click", () => {
    ...
    const desde = document.getElementById("pr_desde").value;
    const hasta = document.getElementById("pr_hasta").value;
    if (!nombre || !desde || !hasta) return toast("Nombre, fecha de inicio y fecha final son obligatorios.", "error");
    if (hasta < desde) return toast("La fecha final debe ser posterior a la fecha de inicio.", "error");
    if (existing){ Object.assign(existing, { nombre, desde, hasta }); }
    else { STATE.academicPeriods.push({ id: uid("per"), nombre, desde, hasta }); }
    persist();
    closeModal();
    toast("Periodo académico guardado", "ok");
  });
}
function deletePeriodo(id){
  STATE.academicPeriods = STATE.academicPeriods.filter(p => p.id !== id);
  persist();
  toast("Periodo académico eliminado", "ok");
}

function setupConfigTab(){
  document.getElementById("newPrefijoBtn").addEventListener("click", newPrefijo);
  document.getElementById("newDocenteBtn").addEventListener("click", newDocente);
  document.getElementById("newPeriodoBtn").addEventListener("click", newPeriodo);
  ...
  document.getElementById("saveDocPassBtn").addEventListener("click", () => {
    const p1 = document.getElementById("cfg_doc_newpass").value;
    const p2 = document.getElementById("cfg_doc_newpass2").value;
    if (!p1) return toast("Escribe la nueva contraseña.", "error");
    if (p1 !== p2) return toast("Las contraseñas no coinciden.", "error");
    STATE.docentePasswordHash = hashStr(p1);
    persist();
    document.getElementById("cfg_doc_newpass").value = "";
    document.getElementById("cfg_doc_newpass2").value = "";
    toast("Contraseña de docentes actualizada", "ok");
  });
...
function renderTab(tabId){
  ...
  const noBanner = tabId === "registro" || tabId === "docentes";
  warn.classList.toggle("hidden", fbConnected || noBanner);
  ...
  if (tabId === "docentes") renderDocentesTab();
  if (tabId === "exportar") renderExportarTab();
}
