...
## Novedades: docentes con acceso individual, periodos académicos y reporte en PDF

- **Docentes con acceso individual** — En **Configuración → Docentes con
  acceso individual** puedes dar de alta a cada docente con su nombre
  (tal como lo escriben sus alumnos en el formulario) y una contraseña
  propia. Al iniciar sesión en "Reporte docentes", cada quien ve
  únicamente la asistencia de sus propios grupos, sin poder cambiar el
  filtro a otro docente. Mientras no des de alta a un docente ahí, sigue
  funcionando la contraseña general de respaldo (Configuración →
  "Contraseña general para docentes").
- **Periodos académicos** — En **Configuración → Periodos académicos**
  registra tus cuatrimestres (por ejemplo, A26 o E27) con su rango de
  fechas. Con al menos dos periodos configurados aparece un comparativo
  lado a lado en **Estadísticas → Comparativo entre periodos académicos**
  (registros, asistencias, % de asistencia y actividad con más demanda),
  y también podrás filtrar el reporte en PDF por periodo.
- **Exportar → Descargar PDF** — Genera un reporte imprimible con el
  detalle de registros y los resúmenes por actividad, por carrera y de
  sanciones. Puedes filtrarlo por periodo académico, actividad, carrera
  o docente antes de generarlo; si no aplicas ningún filtro, incluye todo.
  El Excel, el CSV y el respaldo JSON siempre exportan la información
  completa, sin filtrar.

## Corrección importante: las sesiones no llegaban a los alumnos

Se corrigió un bug en la sincronización con Firebase: cualquier alumno que
abría la app por primera vez (celular u ordenador sin visitas previas)
generaba un estado vacío marcado con la hora exacta en que lo abría, y el
código de sincronización solo aceptaba los datos reales de Firebase si
eran "más recientes" que ese estado vacío — cosa que casi nunca pasaba,
así que el alumno se quedaba sin ver las sesiones programadas. Ahora la
app siempre toma como verdad lo que hay en Firebase, sin comparar fechas.
Con esto, la sincronización entre dispositivos debería funcionar de forma
consistente. Nota: si tú (staff) editas algo sin conexión a internet, al
reconectarte se sobrescribirá con lo último que haya en la nube — evita
hacer cambios importantes sin internet.

## Novedades: paleta institucional y búsqueda de actividad

- **Colores** — Se reemplazó la paleta morado/ámbar/coral original por el
  degradado institucional que pide el documento del proyecto: amarillo
  dorado → naranja → magenta → morado. Se usa en la barra superior, el
  encabezado de "Registro", los botones principales, las tarjetas de
  indicadores (KPI), las etiquetas y las barras de las gráficas. El verde
  (asistencia) y las etiquetas de nivel se conservan tal cual para que
  sigan siendo fáciles de distinguir a simple vista.
- **Buscar actividad en vez de elegir de una lista larga** — En "Registro",
  el estudiante ahora escribe el nombre de la actividad (con sugerencias
  mientras teclea) en lugar de recorrer un menú desplegable con todas las
  sesiones disponibles. Al escribir un nombre que coincide con una
  actividad con cupo, aparece un segundo campo para elegir la fecha y hora
  entre sus sesiones disponibles. También puedes dar clic directamente en
  cualquier tarjeta del catálogo de la derecha para rellenar el buscador
  con esa actividad.
