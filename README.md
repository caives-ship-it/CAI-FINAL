...
## ⚠️ Sobre "algunos alumnos se registran y otros no" (aunque ya diga "Sincronizado")

Encontré y corregí una condición de carrera real: antes, cada vez que
alguien hacía algo (registrarse, escanear asistencia, etc.), la
plataforma subía **todo el estado completo** como un solo bloque a
Firebase. Si dos estudiantes se registraban casi al mismo tiempo desde
dispositivos distintos, el segundo en subir su bloque completo podía
**pisar y borrar sin querer** el registro que el primero acababa de
guardar — aunque ambos vieran "¡Listo, tu registro quedó confirmado!" en
su pantalla.

**La corrección**: ahora cada registro de estudiante se guarda como un
documento independiente en Firebase (no como parte de un bloque
compartido), así que dos estudiantes registrándose al mismo tiempo ya no
pueden pisarse entre sí — cada uno tiene su propio espacio garantizado.
Esto aplica también al escaneo de asistencia y a los demás cambios sobre
registros existentes.

**Nota técnica honesta**: sigue existiendo un caso límite mucho menos
grave: si dos estudiantes compiten por el *último* lugar de una sesión
casi al mismo milisegundo exacto, es posible que ambos pasen la
validación de cupo antes de que se enteren uno del otro, y la sesión
quede con un lugar de más (por ejemplo, cupo 10 con 11 registrados). Esto
es raro (solo ocurre si de verdad coinciden casi al mismo instante) y muy
distinto al problema anterior (que perdía registros silenciosamente);
si te preocupa, dime y puedo agregar una validación más estricta con
transacciones de Firebase.

## ⚠️ Segundo bug encontrado: un dispositivo con configuración vieja "atascado"

Además del bug de la condición de carrera (ver abajo), encontré otro
problema: el orden de prioridad hacía que, si un dispositivo tenía
**cualquier configuración de Firebase guardada localmente** (de una
prueba tuya anterior, incompleta o distinta a la definitiva), esa
configuración vieja le ganaba a la correcta que está incrustada en el
código — y como esa pantalla es solo de staff, **un estudiante no tiene
forma de darse cuenta ni de arreglarlo**. Ya lo corregí: ahora la
configuración incrustada en `data.js` **siempre gana** para todos los
dispositivos, sin excepción.

**Importante para ti:** a partir de ahora, si quieres cambiar el proyecto
de Firebase de forma permanente, ya no basta con hacerlo desde
Configuración → Sincronización en tu navegador — dime los nuevos valores
y yo actualizo `data.js`. Lo que pongas en esa pantalla solo sirve para
pruebas puntuales en tu propio navegador.

## Si sigue sin funcionar en algunos dispositivos después de este fix

Lo más probable, si el problema persiste, es que la **red desde la que se
conectan esos dispositivos esté bloqueando Firebase** (algunas redes
universitarias/escolares filtran dominios de Google por seguridad). Para
confirmarlo:

1. En el dispositivo que falla, revisa el indicador de arriba a la
   derecha: si dice "● Local" de forma persistente (no solo un instante
   al cargar), no se está conectando.
2. Prueba el mismo dispositivo con **datos móviles (4G/5G)** en vez del
   wifi del CAI/universidad. Si ahí sí conecta, confirma que es la red
   wifi la que bloquea Firebase — en ese caso, hay que pedirle a sistemas
   de la universidad que permita los dominios `*.googleapis.com`,
   `*.firebaseio.com` y `*.gstatic.com`.
3. Prueba también desactivando temporalmente cualquier bloqueador de
   anuncios (uBlock, AdBlock, "Shields" de Brave) — algunos bloquean por
   error dominios de Firebase junto con los de rastreo publicitario.
4. Si nada de eso es la causa, abre la consola del navegador (F12 →
   Console) en el dispositivo que falla y mándame la captura del error —
   con eso puedo diagnosticar la causa exacta.
